...
print(a)
print(a[3:8]) # 시작인텍스는 포함, 끝 인덱스는 포함하지 x == range랑 비숫하구나...
print(a)
# 끝 인덱스는 가져오려는 범위에 포함되지 x -> 실제로 가져오려는 인덱스보다 1 더 크게 지정
print(len(a)) # 10 -> 인덱스? 9
print(a[0:10])
print(a[0:100]) # 시작점, 끝점은 벗어나도 어짜피 호출할 수 있는 건
print(a[0]) # 원소(요소 <-> 리스트 x
print(a[0:1]) # 시퀀스[idx:idx+] -> idx의 인덱스의 값만 가진 시퀀스.
print(a[1:1]) # 빈 값
...
print(a[4:7:2]) # 2? --> 인덱스를 2씩 증가하면서 복사해주겠다. --> 끝점 직전까지.
print(a[7:4:-1]) # -1을 넣게 되면? -> 뒤집혀서 들어간다.

# 슬라이스에서 인덱스 생략
'''
슬라이스를 사용할 때 시작 인덱스와 끝 인덱스를 (둘 중하나 혹은 둘 다) 생략될 수 있음.
